-- Ручное применение миграции для добавления KPI полей в teacher view
-- Выполните этот SQL в Supabase Dashboard или через psql
...
-- Проверяем что view создался успешно
SELECT COUNT(*) as teacher_count FROM api_v1.vw_leaderboard_teacher_overall_all;

-- Проверяем структуру view
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_schema = 'api_v1' 
  AND table_name = 'vw_leaderboard_teacher_overall_all' 
