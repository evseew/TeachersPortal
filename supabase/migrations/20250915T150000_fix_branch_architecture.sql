-- ========================================================================
-- АРХИТЕКТУРНОЕ ИСПРАВЛЕНИЕ: Единый источник истины для филиалов
-- ========================================================================
-- Дата: 2025-09-15
...
--
-- Этапы:
-- 1. Синхронизация данных между profiles и teacher_metrics
-- 2. Обновление функции recompute_current_scores()
-- 3. Исправление VIEW vw_leaderboard_teacher_overall_all
-- 4. Подготовка к удалению дублирующего поля (будет в следующей миграции)
-- ========================================================================
...
-- 1. Если profiles.branch_id заполнен - это источник истины
-- 2. Если profiles.branch_id пуст, но teacher_metrics.branch_id заполнен - копируем из metrics в profiles
-- 3. Если оба заполнены но разные - приоритет у profiles (лог конфликта)
...
INSERT INTO sync_log (teacher_id, teacher_name, action, old_metrics_branch, new_metrics_branch, reason)
SELECT 
  p.user_id,
  COALESCE(p.full_name, p.email),
  'SYNC_METRICS_TO_PROFILE',
  COALESCE(old_b.name, 'NULL'),
  COALESCE(new_b.name, 'NULL'),
  'Синхронизирован с profiles.branch_id (источник истины)'
FROM public.profiles p
INNER JOIN public.teacher_metrics tm ON tm.teacher_id = p.user_id
LEFT JOIN public.branch old_b ON old_b.id = (
  -- Получаем старое значение через snapshots или оставляем NULL
  SELECT branch_id FROM public.snapshots 
  WHERE teacher_id = p.user_id 
  ORDER BY created_at DESC 
  LIMIT 1
)
LEFT JOIN public.branch new_b ON new_b.id = tm.branch_id
WHERE tm.updated_at >= now() - interval '1 minute';
...
BEGIN
  ...
  -- (логика создания снимков остается той же)
...
COMMENT ON FUNCTION public.analyze_branch_inconsistencies() IS 'Диагностическая функция для анализа консистентности данных о филиалах';
