-- ========================================================================
-- КОМПЛЕКСНЫЙ АНАЛИЗ АРХИТЕКТУРЫ СИСТЕМЫ (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- ========================================================================
-- Дата: 2025-09-15
...
-- ФУНКЦИЯ СТАТИСТИКИ ПО АРХИТЕКТУРЕ (ИСПРАВЛЕННАЯ)
  ...
  ORDER BY 
    CASE 
      WHEN (SELECT count(*) FROM public.current_scores cs 
            LEFT JOIN public.profiles p ON p.user_id = cs.teacher_id 
            WHERE cs.teacher_id IS NOT NULL AND p.user_id IS NULL) > 0 AND metric_name LIKE '%Orphaned%' THEN 1
      WHEN (SELECT count(*) FROM public.profiles WHERE role = 'Teacher' AND branch_id IS NULL) > 0 AND metric_name LIKE '%без%' THEN 2
      ELSE 3 
    END, metric_name;
$$;
...
SELECT 'КРАТКАЯ СВОДКА ПО ПРОБЛЕМАМ' as section;
SELECT metric_name, value, issue_level, details 
FROM public.get_architecture_summary() 
WHERE (metric_name LIKE '%Orphaned%' OR metric_name LIKE '%Дубли%' OR metric_name LIKE '%без%') 
  AND value > 0;
